...
| `NumberTheory/Erdos1050` | **Erdős #1050** (Borwein 1991/92) — the series `∑_{n≥1} 1/(2ⁿ−3)` is irrational (`erdos_1050`), via Borwein's irrationality criterion for `∑ 1/(qⁿ+r)` specialized to `q=2, r=−3`. The general engine also proves **Borwein's Theorem 1** fully axiom-free — `∑ 1/(qⁿ+c)` is irrational for any integer base of magnitude `≥ 2` and any nonzero rational `c` (`borwein_thm1_abs`), discharging the `borwein_approximants` axiom. Below-the-box variants carry FC-matched names (`erdos_1050.variants.*`, `Variants.lean`): [Er48] with its Lambert `τ(n)`-series identity (proved, `Lambert.lean`), Borwein verbatim (proved), and the **open** [Er88c] transcendence conjecture — a `#guard_msgs`-pinned `sorry` statement with proved faithfulness anchors. | ✅ axiom-clean |
...
- `LeanGallery/NumberTheory/Erdos1050/Statement.lean` — the **headline** `erdos_1050`, stating the
  source's 1-based series `∑_{n≥1} 1/(2ⁿ−3)` verbatim as a literal `tsum` (encoded
  over `ℕ` by the reindex `n ↦ n+1`). `erdos_1050` is the entire trusted
  statement — read it against the erdosproblems.com #1050 statement (Borwein 1991/92). The engine's
