...
/-- The projection `π : ℝ → AddCircle 1` intertwines the real doubling `t ↦ 2t` with the circle
doubling `y ↦ 2•y`: `π (2^n * t) = (y ↦ 2•y)^[n] (π t)`. -/
lemma doubling_iterate_eq (t : ℝ) (n : ℕ) :
...
theorem ae_fract_dense_real :
  ...
  set N : Set (AddCircle (1:ℝ)) :=
    {x | ¬ Dense (Set.range (fun n : ℕ => (fun y : AddCircle (1:ℝ) => (2:ℕ) • y)^[n] x))} with hN
  have hNnull : volume N = 0 := ae_iff.mp ae_dense_orbit_circle
  have hrange : ∀ t : ℝ,
      (Set.range (fun n : ℕ => ((2 ^ n * t : ℝ) : AddCircle (1:ℝ))))
        = Set.range (fun n : ℕ => (fun y : AddCircle (1:ℝ) => (2:ℕ) • y)^[n] ((t : AddCircle (1:ℝ)))) := by
    intro t
    exact congrArg Set.range (funext fun n => doubling_iterate_eq t n)
  rw [ae_iff]
  have hpre :
      {t : ℝ | ¬ Dense (Set.range (fun n : ℕ => ((2 ^ n * t : ℝ) : AddCircle (1:ℝ))))}
        = ((↑) : ℝ → AddCircle (1:ℝ)) ⁻¹' N := by
    ext t
    simp only [Set.mem_setOf_eq, Set.mem_preimage, hN, hrange t]
  rw [hpre]
  have cover : ((↑) : ℝ → AddCircle (1:ℝ)) ⁻¹' N
      ⊆ ⋃ k : ℤ, (((↑) : ℝ → AddCircle (1:ℝ)) ⁻¹' N ∩ Set.Ioc (k:ℝ) ((k:ℝ) + 1)) := by
    intro x hx
    rw [Set.mem_iUnion]
    refine ⟨⌈x⌉ - 1, hx, ?_⟩
    constructor
    ...
    · push_cast
      ...
      linarith
  have hpiece : ∀ k : ℤ,
      volume (((↑) : ℝ → AddCircle (1:ℝ)) ⁻¹' N ∩ Set.Ioc (k:ℝ) ((k:ℝ) + 1)) = 0 := by
    intro k
    have hmp := AddCircle.measurePreserving_mk (1:ℝ) (k:ℝ)
    have hnull := hmp.quasiMeasurePreserving.preimage_null hNnull
